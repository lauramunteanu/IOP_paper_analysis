...
def plot_EnuReco(nEvents, IsReco, IsdCP):
    ...
    # dCP branch is appearance (ν̄μ→ν̄e), use the ν̄e sample (ν̄μ flux × ν̄e σ).
    # dm32 branch is disappearance (ν̄μ→ν̄μ), keep the ν̄μ sample.
    filename = ("../../Remade_April26/nuwro_25031_morestats/DUNE/DUNE_nueb_FSI.flat.root"
                if IsdCP else
                "../../Remade_April26/nuwro_25031_morestats/DUNE/DUNE_numub_FSI.flat.root")
    arr = load_arrays(filename, max_events=(None if nEvents == -1 else nEvents))
    # IsdCP → ν̄e sample (primary lepton is e); not IsdCP → ν̄μ sample (μ).
...
plot_EnuReco(nEvents=1_000_000, IsReco=False, IsdCP=False)
plot_EnuReco(nEvents=1_000_000, IsReco=False, IsdCP=True)
plot_EnuReco(nEvents=1_000_000, IsReco=True,  IsdCP=False)
plot_EnuReco(nEvents=1_000_000, IsReco=True,  IsdCP=True)
