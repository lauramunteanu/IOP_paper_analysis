"""HK dm32 spectrum -- RHC sibling of Fig1_dm32_plot.py.

Uses the HK ν̄μ beam file and computes the antineutrino disappearance
probability P(ν̄μ → ν̄μ) via pmns.SetIsNuBar(True). Same osc-parameter
...
def plot_EnuReco(filename: str, nEvents: int, IsReco: bool):
    ...
    prob_default_numu = np.array([pmns.Prob(1, 1, E/1000.0, L) for E in Enu_t_sel])  # ν̄μ → ν̄μ
    ...
    if(IsReco == True):
        ...
        plt.savefig(outpath("Fig1_plots", "Fig1_EnuQE_dm32_RHC.pdf"))
        plt.close(fig)
        ...
        ax_ratio.set_xlim(0,1200)
        ax_ratio.set_ylim(0.95,1.05)
        plt.savefig(outpath("Fig1_plots", "Fig1_EnuTrue_dm32_RHC.pdf"))
        plt.close(fig)
    return
