"""HK dm32 spectrum -- RHC + relative-energy-shift sibling.

Combines the RHC channel (ν̄μ beam, P(ν̄μ → ν̄μ)) of Fig1_dm32_RHC_plot.py
with the per-event ±0.5%·E_ν^true shift of Fig1_dm32_relshift_plot.py.
...
def plot_EnuReco(filename, nEvents, IsReco):
    ...
    L = 295.0
    pmns.SetPath(L, 2.8)
    pmns.SetIsNuBar(True)   # RHC: P(ν̄μ → ν̄μ)
    pmns.SetMix(theta12, theta23, theta13, deltaCP)
    pmns.SetDeltaMsqrs(dm21, dm32)
    prob_default_numu = np.array([pmns.Prob(1, 1, E / 1000.0, L) for E in Enu_t_sel])

    pmns.SetDeltaMsqrs(dm21, dm32 * 1.004)
    prob_plus_dm2 = np.array([pmns.Prob(1, 1, E / 1000.0, L) for E in Enu_t_sel])

    pmns.SetDeltaMsqrs(dm21, dm32 * 0.996)
    prob_minus_dm2 = np.array([pmns.Prob(1, 1, E / 1000.0, L) for E in Enu_t_sel])
    ...
    if IsReco:
        ...
        plt.savefig(outpath("Fig1_plots", "Fig1_EnuQE_dm32_RHC_relshift.pdf"))
        plt.close(fig)
    else:
        ...
        ax_ratio.set_xlim(0, 1200)
        ax_ratio.set_ylim(0.95, 1.05)
        plt.savefig(outpath("Fig1_plots", "Fig1_EnuTrue_dm32_RHC_relshift.pdf"))
        plt.close(fig)
